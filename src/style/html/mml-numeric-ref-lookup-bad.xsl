...
<xsl:call-template name="index">
  <xsl:with-param name="a"
  select="//att[generate-id()=generate-id(key('att',.)[1])]"/>
...
<xsl:template match="eg/text()">
  <xsl:analyze-string select="." regex="&amp;([#a-zA-Z0-9]+);">
    <xsl:matching-substring>
      <xsl:choose>
	<xsl:when test="regex-group(1)=('lt','gt','amp','quot','apos')">
	  <xsl:value-of select="."/>
	</xsl:when>
	<xsl:otherwise>
	  <xsl:variable name="c" select="key('entity',regex-group(1),doc('../../xml/unicode.xml'))[1]"/>
	  <xsl:text>&amp;#x</xsl:text>
	  <xsl:value-of select="replace(replace($c/@id,'U0*',''),'-0*',';&amp;#x')"/>
	  <xsl:text>;</xsl:text>
	  <span style="color:#999900">[<xsl:value-of select="regex-group(1)"/>]<xsl:text/>
	    <xsl:text>&lt;!--</xsl:text>
...
<xsl:key name="entity" match="character" use="replace(lower-case(@id),'u0*','#x')"/>
<xsl:key name="entity" match="character" use="replace(upper-case(@id),'U0*','#x')"/>
 ...
 <table class="sortable">
   ...
   <tbody>
     <xsl:for-each select="doc('../../xml/unicode.xml')/unicode/charlist/character/operator-dictionary">
       ...
       <tr>
	<xsl:if test="$show.diff.markup='1'  and doc-available('../../xml/unicode-cr.xml')">
	  <xsl:if test="@linebreakstyle or (some $a in key('opdict',concat(../@id,@form),doc('../../xml/unicode-cr.xml'))/@*
