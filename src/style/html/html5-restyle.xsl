...
<xsl:template match="*:nav" priority="222"/>
...
<body>
  ...
  <nav id="toc">
   ...
   <ol class="toc">
  <xsl:text>&#10;</xsl:text>
     <xsl:apply-templates select="doc('chapter1.xml')//nav/ol/li[1]"/>
     <xsl:apply-templates select="(for $c in $chaps return doc(concat($c,'.xml')))//nav/ol/li[last()]"/>
    </ol>
  <xsl:text>&#10;</xsl:text>
  </nav>       
  <xsl:text>&#10;</xsl:text>
...
<xsl:template match="a[.='XHTML+MathML version']" priority="5">
 <a>
