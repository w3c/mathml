...
<xsl:import href="slices-restyle.xsl"/>
...
<xsl:param name="css.base.uri" select="'https://www.w3.org/StyleSheets/TR/2016/'"/>
...
<xsl:template match="coreref[@ref]">
   <a class="coreyes" href="../mathml-core/index.html#{@ref}">core&#x2714;</a>
</xsl:template>
...
<xsl:text>&#10;</xsl:text>
  ...
  <nav id="toc">
   <ol class="toc">
    <li><a href="{$front.filename.base}{$filename.extension}">
     <span class="secno"></span>
     <xsl:text> </xsl:text>
    <xsl:value-of select="/spec/header/title"/></a></li>
  <xsl:if test="$prev">
  <li>
  <a>
              <xsl:attribute name="href">
	      ...
	      <span class="secno"><xsl:apply-templates mode="divnum" select="$prev"/></span>
	      <xsl:text> </xsl:text>
 ...
 <xsl:if test="$next">
  <li> <a>
	      ...
	      <span class="secno"><xsl:apply-templates mode="divnum" select="$next"/></span>
	      <xsl:text> </xsl:text>
